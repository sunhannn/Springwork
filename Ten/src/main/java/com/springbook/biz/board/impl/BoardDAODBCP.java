...
//			+ "values( (select nvl(max(seq),0)+1 from board), ?, ?, ?)";
	private final String BOARD_INSERT = "insert into board(seq, title, writer, content, regdate, cnt, filename) values(?, ?, ?, ?, CURRENT_TIMESTAMP(), 0, ?)";
//	private final String BOARD_INSERT = "insert into board(seq, title, writer, content) values(?, ?, ?, ?)";
	...
	private final String BOARD_UPCNT = "update board SET cnt = cnt+1 WHERE seq = ?";
...
//		jdbcTemplate.update(BOARD_INSERT, vo.getSeq(), vo.getTitle(), vo.getWriter(), vo.getContent());
	...
	// 조회수 업데이트하기 
		public void updateCnt(BoardVO vo) {
			System.out.println("===> Spring JDBC로 조회수 업데이트기능처리");
			Object[] args = { vo.getSeq() };
			jdbcTemplate.update(BOARD_UPCNT, args);
		}
